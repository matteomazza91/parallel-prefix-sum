/**
 *
 *	Title:	Parallel Prefix - Algorithm 2
...
unsigned long dim;
int num_of_threads;
unsigned long num_of_partitions;

float *Vector1, *Vresult;
...
unsigned long computeFirst(unsigned long stage, int threadId){
	...
	unsigned long dim_partition = (dim)/num_of_partitions;
	...
	if(blockId >= 2*num_of_threads) /* if the number of threads is not a power of 2, we'll go in overflow  ((unsigned) -1 is the biggest index)*/
		return -1;
 ...
 */
float getInitialValue(unsigned long stage,int threadId){
	/*
		stage 0:  0 _ 1 _ 2 _ 3 _ 4 _ 5 _ 6 _ 7 _
		stage 1:  _ 0 _ 1 _ 2 _ 3 _ 4 _ 5 _ 6 _ 7
		stage 2:  _ _ 0 1 _ _ 2 3 _ _ 4 5 _ _ 6 7
	...
	*/
	...
	if(stage == 1){
		index = computeLast(stage-1,threadId); 
	}
	else{
		two_power_stage_minus_two <<= stage-2;
		two_power_stage_minus_one = two_power_stage_minus_two << 1;

		index = computeLast(stage-1, threadId - (threadId%two_power_stage_minus_one) + (two_power_stage_minus_two-1 ));
	}
...
void* worker(void* par){
	...
	int id = *((int*)par);
	first = computeFirst(0,id);
	last =  computeLast(0,id);;
	...
	while(stage != -1){
		...
		initialValue = getInitialValue(stage,id);
		...
		if(last != -1){    /*  if needed because of last is typed UNSIGNED ( last can be -1 becouse of the formula, and (unsigned -1) is bigger than first in the second nested if) */
			...
			if(stage < 2){
				if(first <= last){
					if(typeFunc == 0)
						Vector1[first] = g(Vector1[first], initialValue);
					else if(typeFunc == 1)
						Vector1[first] = f(Vector1[first],initialValue,10);
					else if(typeFunc == 2)
						Vector1[first] = f(Vector1[first],initialValue,100);
				}
				...
				for(i = first+1; i <= last; i++){
					...
					if(typeFunc == 0)
						Vector1[i] = g(Vector1[i], Vector1[i-1]);
					else if(typeFunc == 1)
						Vector1[i] = f(Vector1[i],Vector1[i-1],10);
					else if(typeFunc == 2)
						Vector1[i] = f(Vector1[i],Vector1[i-1],100);
				}
			}
				...
				for(i = first; i <= last; i++){
					...
					if(typeFunc == 0)
						Vector1[i] = g(Vector1[i], initialValue);
					else if(typeFunc == 1)
						Vector1[i] = f(Vector1[i],initialValue,10);
					else if(typeFunc == 2)
						Vector1[i] = f(Vector1[i],initialValue,100);
				}
			}
		}
